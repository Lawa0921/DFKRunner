...
async function main() {
  for (let i = 0; i < config.walletAddressAndPrivateKeyMappings.length; i++) {
    ...
    rentedRecords.forEach((rentedRecord) => {
      console.log(`${config.walletAddressAndPrivateKeyMappings[i].accountName} ${new Date(rentedRecord.endedAt * 1000).toLocaleTimeString()} rented ${rentedRecord.tokenId.id} ${BigInt(rentedRecord.purchasePrice) / BigInt(10 ** 18)} C`)
    })
  }  
}
